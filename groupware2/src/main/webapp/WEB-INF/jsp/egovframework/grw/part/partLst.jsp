...
<div class="card border-grey-300 my-0 mb-1">
	<div class="card-header bg-light header-elements-inline pl-2 py-2">
		<h6 class="card-title font-weight-bold font-size-sm">부서 목록</h6>
	</div>
						...
						<c:when test="${not empty pagingInfo.dataList}">
							<c:forEach items="${pagingInfo.dataList}" var="item" varStatus="status">
								<tr>
									...
									<td class="text-left">${item.partNm}</td>
									<td class="text-center">
										<input type="hidden" name="partId" value="${item.partId }" />
										<a href="/grw/part/partUpd.do?partId=${item.partId }" class="btn btn-outline bg-dark border-grey-800 text-dark">편집</a>
...
<div class="row">
	<div class="col-12 col-md-6 mt-md-0 mt-1 text-right">
		<a href="/grw/part/partIns.do" class="btn btn-outline bg-dark border-grey-800 text-dark">추가</a>
	</div>
</div>